...
این سرور روی همان پروسه ربات اجرا می‌شود و callback را دریافت می‌کند
"""
import asyncio
import threading
    ...
    def handle_zibal_callback(self, parsed):
        ...
        if not payment:
            ...
            self.send_result_page(
                "نامعتبر",
                "این تراکنش در سیستم ثبت نشده است."
            )
            return
        ...
        if payment["status"] == "failed":
            ...
            self.send_result_page(
                "ناموفق",
                "این تراکنش قبلاً ناموفق ثبت شده است."
            )
            return
        ...
        if success_param != "1" or status_param != "2":
            print(
                ...
                f"success={success_param} status={status_param}"
            )
        ...
        paid_amount_rial = verify_result.get(
            "amount_rial",
            0
        )
        ...
        if paid_amount_rial != expected_amount_rial:
            print(
                f"🔴 Amount mismatch! "
                f"paid={paid_amount_rial} "
                f"expected={expected_amount_rial}"
            )
        ...
        ref_id = verify_result.get(
            "ref_id",
            "-"
        )

        card_pan = verify_result.get(
            "card_pan",
            "-"
        )
        ...
        if package_id is not None:
            ...
            if pkg:
                ...
                if user:
                    ...
                    expire_date = get_shamsi_future_date(
                        pkg["days"]
                    )
                    ...
                    if _bot_instance:
                        try:
                            ...
                            print(
                                f"🔴 Error sending channel report: {e}"
                            )

        self.send_result_page(
            "موفق",
            "پرداخت شما با موفقیت انجام شد. لطفاً به ربات برگردید.",
            success=True,
        )
...
async def send_success_message(
    ...
    try:
        ...
        print(
            f"🔴 send_success_message error: {e}"
        )
...
def run_server():
    ...
    print(
        f"🌐 Payment server running "
        f"on port {WEB_PORT}"
    )

    print(
        f"🔗 Callback URL: "
        f"https://{RAILWAY_PUBLIC_DOMAIN}/zibal/callback"
    )
